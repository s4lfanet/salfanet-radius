...
import { apiAgent } from '@/lib/api/client';
...
export default function AgentPushToggle() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (sub) {
        ...
        if (token) {
          fetch('/api/push/agent-subscribe', {
            method: 'POST',
            headers: {
      ...
      console.warn('[AgentPush] Service worker registration failed:', e);
    }
  }, []);
  ...
  const handleToggle = async () => {
    ...
    try {
      const token = localStorage.getItem('agentToken');
      if (!token) {
        addToast({ type: 'error', title: 'Sesi berakhir', description: 'Silakan login kembali' });
        return;
      }
      ...
      if (subscribed) {
        ...
        await fetch('/api/push/agent-unsubscribe', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
        ...
        const subRes = await fetch('/api/push/agent-subscribe', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
