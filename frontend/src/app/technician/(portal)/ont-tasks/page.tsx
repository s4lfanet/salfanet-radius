    ...
    <div className="p-4 lg:p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-[#e0d0ff]/60">{filtered.length} tugas</p>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari username / nama..." className="w-full pl-10 pr-3 py-2.5 bg-white dark:bg-[#0a0520] border border-slate-200 dark:border-[#bc13fe]/30 rounded-xl text-sm text-slate-900 dark:text-white placeholder:text-slate-400 focus:ring-2 focus:ring-[#00f7ff]/30 transition" />
        </div>
        <div className="flex gap-1">
          {([
            ...
            <button key={f.key} onClick={() => setStatus(f.key)} className={`px-3 py-2 text-xs font-bold rounded-xl transition ${status === f.key ? 'bg-[#bc13fe] text-white shadow-[0_0_15px_rgba(188,19,254,0.4)]' : 'bg-slate-100 dark:bg-[#1a0f35] border border-slate-200 dark:border-[#bc13fe]/20 text-slate-600 dark:text-[#e0d0ff]/70 hover:bg-slate-200 dark:hover:bg-[#bc13fe]/10'}`}>
              {f.label}
            </button>
          ))}
        </div>
      ...
      {loading && tasks.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((task) => (
            <div key={task.id} className="bg-white dark:bg-[#1a0f35]/80 border border-slate-200 dark:border-[#bc13fe]/20 rounded-2xl overflow-hidden">
              <div className="p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-bold text-slate-900 dark:text-white">{task.username}</p>
                    <p className="text-xs text-slate-500 dark:text-[#e0d0ff]/60">{task.customerName}{task.customerId ? ` (${task.customerId})` : ''}</p>
                  </div>
                ...
                {task.address && (
                  <p className="text-xs text-slate-500 dark:text-[#e0d0ff]/60 flex items-center gap-1">
                ...
                {task.reason && (
                  <p className="text-xs text-slate-600 dark:text-[#e0d0ff]/80 italic">&quot;{task.reason}&quot;</p>
                )}
              </div>
                  ...
                  {activeId === task.id ? (
                    <>
                      <input
                        ...
                        className="w-full px-3 py-2 bg-white dark:bg-[#1a0f35] border border-slate-200 dark:border-[#bc13fe]/30 rounded-lg text-sm text-slate-900 dark:text-white placeholder:text-slate-400"
                      />
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 bg-slate-200 dark:bg-[#1a0f35] text-slate-600 dark:text-[#e0d0ff]/70 text-xs font-bold rounded-lg transition"
                        >
                          Tutup
                        </button>
                      </div>
