      ...
      <div className="flex flex-col sm:flex-row gap-2">
        ...
        <div className="flex gap-1">
          {(['all', 'online', 'offline'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} className={`px-3 py-2 text-xs font-bold rounded-xl transition ${filter === f ? 'bg-violet-500 text-white ' : 'bg-slate-100 dark:bg-muted border border-border text-muted-foreground hover:bg-slate-200 dark:hover:bg-primary/10'}`}>
        ...
        <>
          ...
          <div className="hidden lg:block overflow-auto bg-card border border-border rounded-2xl">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((u) => (
                  <tr key={u.id} onClick={() => setExpandedUser(expandedUser === u.id ? null : u.id)} className="border-b border-slate-100 dark:border-border hover:bg-slate-50 dark:hover:bg-primary/10 transition cursor-pointer">
