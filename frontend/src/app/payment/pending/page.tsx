  ...
  if (error || !invoice) return (
    <div className="min-h-dvh bg-muted relative overflow-hidden flex items-center justify-center p-4">
      ...
      <div className="relative z-10 max-w-sm w-full bg-muted/80 backdrop-blur-xl rounded-2xl border-2 border-red-500/50 p-6 text-center shadow-[0_0_40px_rgba(255,68,102,0.2)]">
        ...
        <button onClick={() => router.push('/')} className="px-6 py-2.5 text-xs font-bold bg-gradient-to-r from-violet-500 to-brand-500 text-white rounded-xl">Kembali</button>
      </div>
    </div>
  );
      ...
      <div className="max-w-md mx-auto space-y-4 relative z-10">
        <div className="text-center">
          <div className="inline-block relative mb-4">
            <div className="absolute inset-0 bg-pink-500/30 rounded-full animate-pulse"></div>
            <div className="relative w-20 h-20 bg-gradient-to-br from-pink-500 to-violet-500 rounded-full flex items-center justify-center mx-auto shadow-[0_0_40px_rgba(236,72,153,0.5)]">
              <Clock className="w-10 h-10 text-white animate-bounce " />
            </div>
          </div>
          <h1 className="text-xl font-bold text-pink-500 mb-1">Menunggu Pembayaran</h1>
          <p className="text-xs text-muted-foreground/70">Pembayaran sedang diproses</p>
        </div>

        <div className="bg-muted/80 backdrop-blur-xl rounded-2xl border-2 border-pink-500/50 overflow-hidden shadow-[0_0_40px_rgba(236,72,153,0.2)]">
          <div className="bg-gradient-to-r from-pink-500 to-violet-500 px-4 py-3">
          ...
          <div className="p-5 space-y-4">
            <div className="text-center py-5 bg-gradient-to-br from-pink-500/10 to-violet-500/10 rounded-xl border border-pink-500/20">
            ...
            <div className="bg-pink-500/10 border border-pink-500/30 rounded-xl p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="text-xs text-muted-foreground/80 space-y-1.5">
                  <p className="font-bold text-pink-500">Langkah Selanjutnya:</p>
            ...
            <div className="flex items-center justify-center gap-2 text-[10px] text-muted-foreground/60">
              <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse shadow-[0_0_10px_rgba(0,255,136,0.5)]"></div>
              <span>Auto-refresh setiap 5 detik</span>
            </div>

            {invoice.paymentLink && (
              <a href={invoice.paymentLink} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-2 w-full px-4 py-3 text-sm font-bold text-white bg-gradient-to-r from-violet-500 to-brand-500 rounded-xl shadow-[0_0_25px_rgba(139,92,246,0.4)] hover:shadow-[0_0_35px_rgba(139,92,246,0.6)] transition-all">
        ...
        <div className="space-y-2">
          <button onClick={() => { setChecking(true); fetchInvoiceStatus(); }} disabled={checking} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-xs font-medium bg-card text-white rounded-xl border-2 border-violet-500/30 hover:border-brand-500 disabled:opacity-50">
            <RefreshCw className={`w-4 h-4 text-brand-500 ${checking ? 'animate-spin' : ''}`} />{checking ? 'Mengecek...' : 'Cek Status'}
          </button>
          <button
            onClick={() => {
  ...
  return (
    <Suspense fallback={
      <div className="min-h-dvh bg-muted flex items-center justify-center">
        <Loader2 className="w-10 h-10 animate-spin text-brand-500 drop-shadow-[0_0_20px_rgba(6,182,212,0.6)]" />
      </div>
    }>
