...
import { Plus, Pencil, Trash2, MapPin, Users, Search, X } from 'lucide-react';
...
export default function AreasPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]"><div className="absolute inset-0 overflow-hidden pointer-events-none"><div className="absolute top-1/4 left-1/4 w-48 h-48 sm:w-96 sm:h-96 bg-primary/10 rounded-full blur-3xl animate-pulse"></div><div className="absolute bottom-1/4 right-1/4 w-48 h-48 sm:w-96 sm:h-96 bg-brand-500/20 rounded-full blur-3xl animate-pulse delay-1000"></div></div><MapPin className="w-12 h-12 text-brand-500 dark:text-brand-500 dark:drop- relative z-10 animate-pulse" /></div>
    );
  }
    ...
    <div className="bg-background relative">
      ...
      <div className="relative z-10 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-500 dark:via-white dark:to-pink-500 dark:drop-">{t('pppoe.areasTitle')}</h1>
        ...
        <div className="block md:hidden space-y-3">
          {filteredAreas.length === 0 ? (
            <div className="bg-card rounded-xl border border-border p-6 text-center text-muted-foreground text-xs">
              {areas.length === 0 ? t('pppoe.noArea') : t('pppoe.noMatchArea')}
            </div>
          ) : (
            filteredAreas.map((area) => (
              <div key={area.id} className="bg-card rounded-xl border border-border p-3">
        ...
        <SimpleModal isOpen={isDialogOpen} onClose={() => { setIsDialogOpen(false); setEditingArea(null); resetForm(); }} size="md">
          ...
          <form onSubmit={handleSubmit}>
            <ModalBody className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" id="isActive" checked={formData.isActive} onChange={(e) => setFormData({ ...formData, isActive: e.target.checked })} className="rounded border-border bg-background dark:bg-card accent-brand-500 dark:accent-brand-500 w-4 h-4" />
