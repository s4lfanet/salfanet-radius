          ...
          <div className="bg-card rounded-lg border border-border shadow-sm">
            ...
            <div className="block md:hidden space-y-3 p-4">
              ...
              {backupHistory.length === 0 ? (
                <div className="bg-card/80 backdrop-blur-xl rounded-xl border border-violet-500/20 p-3 text-center text-sm text-muted-foreground">
                  No backup history yet
                </div>
              ) : (
                backupHistory.map((backup) => (
                  <div key={backup.id} className={`bg-card/80 backdrop-blur-xl rounded-xl border p-3 transition ${
                    selectedIds.has(backup.id) ? 'border-primary/50 bg-primary/5' : 'border-violet-500/20'
                  }`}>
