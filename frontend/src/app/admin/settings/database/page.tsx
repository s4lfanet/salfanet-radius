...
export default function DatabaseSettingsPage() {
  ...
  const handleBackupNow = async () => {
    ...
    setBacking(true);
    try {
      const data = await apiAdmin<{ success: boolean; downloadUrl?: string; filename?: string; error?: string }>('/api/backup/create', { method: 'POST' });
      ...
      await showError(t('settings.createBackupFailed') + ': ' + (error instanceof Error ? error.message : String(error)));
    } finally {
      setBacking(false);
    }
  };
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      const data = await apiAdmin<{ success: boolean; error?: string }>('/api/backup/restore', {
        method: 'POST',
        body: formData,
      });
      ...
      await showError(t('settings.failedRestore') + ': ' + (error instanceof Error ? error.message : String(error)));
    } finally {
      setRestoring(false);
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    for (const id of selectedIds) {
      try {
        const data = await apiAdmin<{ success: boolean }>(`/api/backup/delete/${id}`, { method: 'DELETE' });
  ...
  const handleDeleteBackup = async (id: string, filename: string) => {
    ...
    try {
      const data = await apiAdmin<{ success: boolean; error?: string }>(`/api/backup/delete/${id}`, { method: 'DELETE' });
      ...
      await showError(t('settings.deleteBackupFailed') + ': ' + (error instanceof Error ? error.message : String(error)));
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <div className="relative z-10 text-center">
          <Shield className="w-16 h-16 text-[#ff3366] drop- mx-auto mb-4" />
          <h2 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-500 dark:via-white dark:to-pink-500 mb-2">
            Access Denied
          </h2>
    ...
    <div className="bg-background relative">
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-500 dark:via-white dark:to-pink-500 dark:drop-">
          <Database className="w-6 h-6 text-brand-500 inline mr-2" />
          Database Management
        </h1>
          ...
          <div className="bg-card rounded-lg border border-border shadow-sm">
            ...
            <div className="block md:hidden space-y-3 p-4">
              ...
              {backupHistory.length === 0 ? (
                <div className="bg-card rounded-xl border border-border p-3 text-center text-sm text-muted-foreground">
                  No backup history yet
                </div>
              ) : (
                backupHistory.map((backup) => (
                  <div key={backup.id} className={`bg-card rounded-xl border p-3 transition ${
                    selectedIds.has(backup.id) ? 'border-primary/50 bg-primary/5' : 'border-border'
                  }`}>
