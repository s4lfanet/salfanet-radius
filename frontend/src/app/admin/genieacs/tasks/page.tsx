    ...
    <div className="bg-background relative">
      ...
      <div>
        <h1 className="text-xl sm:text-2xl font-bold text-foreground dark:text-transparent dark:bg-clip-text dark:bg-gradient-to-r dark:from-brand-500 dark:via-white dark:to-pink-500 dark:drop- flex items-center gap-2">
          <ListTodo className="w-6 h-6 text-brand-500 dark:text-brand-500 dark:drop-" />
        ...
        <div className="block md:hidden space-y-3 p-3">
          {filteredTasks.length === 0 ? (
            ...
            filteredTasks.map((task) => (
              <div key={task._id} className="bg-card rounded-xl border border-border p-3">
              ...
              {filteredTasks.length === 0 ? (
                ...
                filteredTasks.map((task) => (
                  <tr key={task._id} className="border-b border-border hover:bg-muted">
                    ...
                    <td className="py-2 px-3 text-muted-foreground whitespace-nowrap truncate overflow-hidden">
                      {formatWIB(task.timestamp, 'dd/MM/yyyy HH:mm')}
                    </td>
