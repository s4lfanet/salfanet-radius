  ...
  return (
    <div className="min-h-dvh bg-gray-50 dark:bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-foreground">{t('network.diagram.title')}</h1>
                  ...
                  {otbList.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
                      ...
                      <div className="xl:col-span-2 space-y-4">
                        <div className="flex items-center gap-4 bg-card p-4 rounded-lg border border-border">
                          ...
                          <select
                            ...
                            className="flex-1 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-foreground focus:ring-2 focus:ring-blue-500"
                          >
                            {otbList.map(o => (
                      ...
                      <div className="bg-card rounded-lg border border-border p-4 space-y-4">
                        <h3 className="font-semibold text-foreground text-sm">Penugasan Tabung → JC</h3>
                        ...
                        {(otbDetail?.outputSegments?.length ?? 0) > 0 ? (
                            ...
                            <table className="w-full text-xs">
                              <thead>
                                <tr className="border-b border-border">
                                  <th className="text-left pb-2 text-muted-foreground">Tabung</th>
                                  <th className="text-left pb-2 text-muted-foreground">JC</th>
                                  <th className="text-left pb-2 text-muted-foreground">Status</th>
                                  <th className="pb-2" />
                                </tr>
                              </thead>
                              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                {otbDetail?.outputSegments?.map((seg) => (
                                  <tr key={seg.id}>
                                    <td className="py-1.5 font-mono text-muted-foreground">T{seg.fromPort}</td>
                          ...
                          <p className="text-xs text-muted-foreground/70">Belum ada tabung yang ditugaskan ke JC.</p>
                        )}

                        {/* Add assignment form */}
                        <div className="border-t border-border pt-4 space-y-2">
                          <p className="text-xs font-medium text-muted-foreground">Tugaskan Tabung ke JC</p>
                          <div className="flex gap-2">
                            <input
                              ...
                              className="w-24 px-2 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-foreground"
                            />
                            <select
                              value={assignJc}
                          ...
                          <input
                            ...
                            className="w-full px-2 py-1.5 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-foreground"
                          />
                          <button
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="bg-card rounded-lg border border-border p-4">
                          ...
                          {(jcDetail?.inputSegments?.length ?? 0) === 0 ? (
                            <p className="text-xs text-muted-foreground/70">Belum ada kabel yang masuk.</p>
                          ) : (() => {
                            // Group by cable
                        ...
                        <div className="bg-card rounded-lg border border-border p-4">
                          ...
                          {(jcDetail?.outputSegments?.length ?? 0) === 0 ? (
                            <p className="text-xs text-muted-foreground/70">Belum ada kabel yang keluar.</p>
                          ) : (() => {
                            const grouped = new Map<string, { cable: JcSegment['fiber_cables']; segments: JcSegment[] }>();
                            for (const seg of (jcDetail?.outputSegments ?? [])) {
                              const cid = seg.fiber_cables?.id ?? seg.cableId ?? '';
                        ...
                        {/* Splice summary */}
                        <div className="bg-card rounded-lg border border-border p-4">
                          <h3 className="font-semibold text-foreground text-sm mb-2">Splice Points</h3>
                    ...
                    <>
                      <div className="flex items-center gap-4 bg-card p-4 rounded-lg border border-border">
                        ...
                        <select
                          ...
                          className="flex-1 bg-white dark:bg-gray-700 text-foreground border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-cyan-500"
                        >
                          {odcList.map((odc) => (
                    ...
                    <>
                      <div className="flex items-center gap-4 bg-card p-4 rounded-lg border border-border">
                        ...
                        <select
                          ...
                          className="flex-1 bg-white dark:bg-gray-700 text-foreground border border-gray-300 dark:border-gray-600 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500"
                        >
                          {odpList.map((odp) => (
