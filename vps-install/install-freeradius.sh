...
sql {
    ...
    # read_clients = no — NAS clients are managed via clients.d/nas-from-db.conf
    # (generated by the app from the `nas` table with VPN gateway support).
    # Setting read_clients=yes here would cause "Failed to add duplicate client"
    # because the same NAS is defined in both the `nas` table AND clients.d/.
    read_clients = no
    client_table = "nas"
