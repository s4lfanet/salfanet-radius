...
### Fixed
- **Telnet Test Connection selalu timeout (30s)** — Script expect sebelumnya menunggu prompt `#`/`>` lalu menjalankan `display version` yang bisa hang. Diganti dengan script yang lebih robust: TCP port check 3s dulu, kemudian expect yang menunggu pola prompt manapun (`#`, `>`, `# SALFANET RADIUS - Billing System for ISP/RTRW.NET
...
, atau username/password prompt) dan langsung exit. Total timeout 15s, tidak perlu menjalankan perintah CLI apapun.
